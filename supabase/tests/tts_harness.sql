-- =====================================================================
-- Tournament Testing Suite (TTS) — reusable pgTAP lifecycle harness.
--
-- Source of truth: docs/plans/tournament-testing-suite/SPEC.md (T-Harness).
-- This file defines the §3 helper functions (all prefixed `_tts_` to avoid
-- catalog collisions) that drive the CURRENTLY BUILT tournament lifecycle
-- end-to-end through the real RPCs and triggers, plus the §4 assertion
-- helpers and the §3.7 per-row driver. It is SELF-CONTAINED and runnable
-- standalone under `supabase test db`: everything is wrapped in
-- BEGIN .. ROLLBACK, the helpers are CREATE OR REPLACE inside the
-- transaction (auto-dropped on ROLLBACK — NO production migration), and a
-- structural plan() asserts every §3 helper exists before finish().
--
-- IST-vs-WISH (SPEC §0.2): only the currently built contract is driven.
-- max_participants stays <= 200 (server clamp, tournament_create_setup.sql
-- line 64), qualifier_count is a power of two in [2,64], player sizes are
-- N in {32,48,60}. No 1000-player path, no year-suffix, no diggy-default-on.
--
...
-- (cf. tournament_ko_rpcs.sql, pair_round_swiss.test.sql).
-- =====================================================================
...
-- =====================================================================
-- Structural self-check: assert every §3 helper exists, so this file is a
-- valid, runnable pgTAP file on its own. 14 named helpers per SPEC §3.1-§3.7.
-- =====================================================================
SELECT plan(14);

SELECT has_function('_tts_config'::name);
SELECT has_function('_tts_seed_tournament'::name);
SELECT has_function('_tts_register_n'::name);
SELECT has_function('_tts_participant'::name);
SELECT has_function('_tts_target'::name);
SELECT has_function('_tts_start'::name);
SELECT has_function('_tts_play_round'::name);
SELECT has_function('_tts_advance'::name);
SELECT has_function('_tts_assert_placement_dense'::name);
SELECT has_function('_tts_assert_bracket_consistent'::name);
SELECT has_function('_tts_assert_schedule_timestamps'::name);
SELECT has_function('_tts_assert_notifications'::name);
SELECT has_function('_tts_assert_target_reached'::name);
SELECT has_function('_tts_run'::name);
