-- Tournament feature — CF5 (K28): project `club_id` from tournament_get.
--
-- The tournament detail screen surfaces the organizing club / Spasstournier
-- category, which is driven by `TournamentDetailHeader.clubId`. The wire
-- parser (`tournamentDetailHeaderFromRow`) already reads `row['club_id']`, but
-- the previous projection (20261201000021_tournament_get_setup_fields) never
-- emitted it, so the field was always null on the client.
--
-- Purely additive: CREATE OR REPLACE re-declares the function with ONE extra
-- read-only jsonb key (`club_id`) in the `tournament` block. The
-- participants / matches / audit blocks and the access gate are unchanged
-- (copied verbatim from 20261201000021). No schema/column change. GRANT is
-- re-applied so the authenticated role keeps EXECUTE.
