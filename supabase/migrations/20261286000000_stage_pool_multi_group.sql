-- Tournament — ADR-0033 §4 P5.3a: stage POOL nodes honour multi-group config.
--
-- The stage generator's pool/round_robin branch previously emitted a single
-- flat group. This re-bases tournament_generate_stage_matches from its latest
-- body (20261261000000) and, additively, splits the seeded field into
-- group_count groups via _tournament_compute_pools (snake/seeded/random),
-- emitting intra-group round-robin pairs tagged with group_label. group_count<=1
-- keeps the original single flat group (group_label NULL) — behaviour-stable.
-- Pure CREATE OR REPLACE of one function; no schema/CDC/publication change.
