-- Round 1 of a stage-graph tournament ignored the pitch plan.
--
-- Since ADR-0039 §4 every schoch/swiss tournament starts through
-- tournament_start_stage_graph. That branch of tournament_start returns right
-- after delegating, so neither of tournament_start's _tournament_assign_pitches
-- calls is reached, and tournament_start_stage_graph never had one of its own.
-- Result: a tournament configured for pitches 16..32 played its whole first
-- round on pitch 1. Group-phase tournaments were unaffected (they go through
-- tournament_start_pool_phase), and so were rounds 2+ (tournament_pair_round
-- assigns).
--
-- Verbatim re-base of the body from 20261299000000_stage_seed_resolver.sql with
-- the single PERFORM added before the go-live update.
...
BEGIN
  ...
  -- Apply the pitch plan to the freshly booted round 1. The schoch/swiss route
  -- reaches 'live' through here and returns before tournament_start's own two
  -- assignment calls, so without this every first-round match keeps the
  -- inserted default of pitch 1 no matter what the organiser configured.
  -- Follow-up rounds are already covered by tournament_pair_round.
