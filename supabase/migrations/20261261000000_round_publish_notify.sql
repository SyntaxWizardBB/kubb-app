-- Phase C (ADR-0031) Block C1 — per-pitch publish-notify (E1 "Runde N
-- publiziert") wired into the FIVE materialisation RPCs, incl. the
-- generate_stage_matches gap-close (which was previously silent).
--
-- ============================ K3 RE-BASE DISCIPLINE ======================
-- C runs AFTER A1 has landed, so the GENUINE latest on-disk body of each of
-- the five RPCs is now migration 20261252000000_round_schedule_materialize.sql
-- (the A1 version that ALREADY carries the
--   PERFORM public._tournament_upsert_round_schedule(...)
-- schedule line[s]). Verified per function via
--   grep -rln 'FUNCTION public.<fn>(' supabase/migrations/ | sort | tail -1
--   -> all five resolve to 20261252000000 (A1).
-- Each body below is therefore RE-BASED on the …252 source — NOT on the
-- pre-A versions (…040 / …210 / …032 / …247 / …226). The A1 schedule PERFORM
-- lines are PRESERVED verbatim; the ONLY functional difference vs the …252
-- source body is the inserted per-pitch publish-notify call(s):
--   PERFORM public._tournament_notify_round_per_pitch(
--     <tournament_id>, <round_number>, <phase>, 'round_published', <subj>, <body>);
--
-- The C0 helper _tournament_notify_round_per_pitch(uuid,int,text,text,text,text)
-- (migration 20261260000000) is ONLY CALLED here, never re-defined. It resolves
-- starts_at INTERNALLY from tournament_round_schedule (A's row); when no
-- schedule row exists (A degraded/absent) it omits the "Start HH:MM" segment
-- while the "Pitch X" segment stays — so C1 is SOFT-dependent on A (no
-- starts_at argument is passed here). It also owns the OD-3 "Pitch X only when
-- a pitch is actually assigned" guard (pitch_number > 0) and the per-recipient
-- idempotency NOT EXISTS guard keyed on
-- (tournament_id, round_number, action_payload.kind='round_published', user_id)
-- scoped to per-pitch rows — so a re-publish / double materialise inserts no
-- duplicate inbox rows.
--
-- Wire-kind stays 'tournament_round'; the schedule sub-event tag is
-- 'round_published' in action_payload.kind. NO new wire-kind, NO change to the
-- user_inbox_messages kind CHECK (that is C0 scope). PII-free: the payload is
-- the C0 6-key whitelist only (tournament_id, round_number, phase, starts_at,
-- pitch_number, kind) — C1 builds no payload of its own.
--
-- Each notify is placed AFTER matches + pitches (_tournament_assign_pitches)
-- and the schedule row (_tournament_upsert_round_schedule) exist, so
-- pitch_number is already assigned and resolvable and starts_at (if any) is on
-- disk for the helper to read.
--
-- Idempotent / additive: CREATE OR REPLACE only; no DROP/TRUNCATE/DELETE, no
-- db reset, no destructive DDL.


-- ====================================================================
-- 1. tournament_start — re-based from 20261252000000 (A1; itself …040
--    Open-Registration). swiss/schoch -> round 1; round_robin -> round 1
--    (the active round). C1: per-pitch publish-notify (round 1, phase
--    'group') after the round-1 schedule row exists.
-- ====================================================================
...
-- ====================================================================
-- 2. tournament_start_pool_phase — re-based from 20261252000000 (A1).
--    C1: per-pitch publish-notify (round 1, phase 'group').
-- ====================================================================
...
-- ====================================================================
-- 3. tournament_pair_round — re-based from 20261252000000 (A1).
--    C1: per-pitch publish-notify of the newly paired v_next_round
--    (phase 'group').
-- ====================================================================
...
-- ====================================================================
-- 4. tournament_start_ko_phase — re-based from 20261252000000 (A1; CF6
--    seeding gate + SHOOTOUT-GATE/-RESOLVE + Consolation PRESERVED). C1:
--    one per-pitch publish-notify per KO round in the EXISTING pitch loop,
--    phase 'final' for the max round else 'ko' (mirrors the schedule-row
--    phase).
-- ====================================================================
...
COMMENT ON FUNCTION public.tournament_start_ko_phase(uuid, jsonb) IS
  ...
  '(phase final for the last round, else ko); ADR-0031 C1 adds a per-pitch '
  'publish-notify per KO round in the same loop (round_published, phase '
  'final/ko). Otherwise unchanged: SHOOTOUT Gate/Resolve, Pool-Cut/Standings '
  '+ Overrides, double_elimination, Consolation, tournament_caller_can_manage '
  'auth, Pitch-Plan, Go-Live-Notify, idempotent 40001 path, seeding_required '
  'gate (22023). Siehe ChangeSpec K19.';


-- ====================================================================
-- 5. tournament_generate_stage_matches — re-based from 20261252000000 (A1).
--    GAP-CLOSE (C1): this RPC fired NO participant notify at all (per
--    …247/…252), so stage-graph rounds were SILENT. C1 adds a per-pitch
--    publish-notify (round 1, phase 'group' — consistent with the stage's
--    materialised round) AFTER the matches and the stage schedule row exist.
-- ====================================================================
