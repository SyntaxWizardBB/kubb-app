-- double_elim + consolation AFTER schoch — ADR-0039 §4 (Unit 8b C/D/E).
--
-- The phase-inclusive slot index (20261315000000) lets a stage materialise a
-- double_elim bracket (three phase-distinct rows per slot) and a consolation
-- sub-bracket. Three functions are re-based to actually drive them:
--
--   C) tournament_start — the schoch_then_ko / swiss_then_ko auto-route now
--      derives the KO stage TYPE from tournaments.bracket_type instead of
--      hard-coding single_elim:
--        * single_elimination  -> 'single_elim'  (byte-identical to 20261313).
--        * double_elimination  -> 'double_elim', stage config carries with_reset
--          from ko_config.with_bracket_reset (owner default OFF, written EXPLICIT).
--        * consolation enabled -> 'consolation', stage config mirrors the
--          consolation_bracket (source early_ko_losers, main_bracket_size
--          next_pow2(qualifier_count), direct_count).
--      Seeding stays from_prev_ranking. Every other branch is byte-identical.
--
--   D) tournament_generate_stage_matches — a consolation stage no longer behaves
--      like single_elim. It still builds the MAIN single-elim bracket via
--      _tournament_compute_ko_bracket, then ADDITIONALLY materialises the
--      consolation sub-bracket via _tournament_compute_cons_bracket (mirroring
--      tournament_start_ko_phase: main_size = next_pow2(qualifier_count),
--      direct_count, phase 'consolation'); the early_ko_losers are routed in by
--      the advance trigger at runtime. The single_elim path stays BYTE-IDENTICAL.
--
--   E) tournament_advance_ko_winner — the DE and consolation advance/drop
--      branches are scoped to the firing match's stage with
--      `AND stage_node_id IS NOT DISTINCT FROM NEW.stage_node_id`, so a winner
--      lands only in its own stage bracket. The NULL case (classic single-KO /
--      start_ko_phase / round_robin_then_ko, stage_node_id NULL) is unchanged
--      (NULL IS NOT DISTINCT FROM NULL = true).
--
-- _tournament_compute_de_bracket / _compute_cons_bracket / _compute_ko_bracket
-- are NOT changed — only called correctly. The type_graph pre-dispatch guard
-- (20261305) is not touched.

-- ===================================================================
-- C) tournament_start — re-based from 20261313000000. Only the
-- schoch_then_ko / swiss_then_ko KO-stage derivation changed (bracket_type /
-- consolation aware). Every other branch is byte-identical.
-- ===================================================================
...
DECLARE
  ...
  -- Unit 8b: KO-type derivation from bracket_type / consolation_bracket.
  v_bracket_type   text;
  v_cons_cfg       jsonb;
  v_cons_enabled   boolean;
  v_with_reset     boolean;   -- double_elim opt-in, owner default OFF
  v_cons_main_size int;
  v_cons_direct    int;
  v_ko_cfg         jsonb;     -- assembled KO stage config
BEGIN
  v_caller := auth.uid();
  ...
  SELECT status, format, pool_phase_config, ko_config, ko_matchup,
         display_name, created_by, bracket_type, consolation_bracket
    INTO v_status, v_format, v_pool_config, v_ko_config, v_ko_matchup,
         v_name, v_created_by, v_bracket_type, v_cons_cfg
  ...
  IF v_format IN ('schoch_then_ko','swiss_then_ko') THEN
    ...
    IF v_stage_count = 0 THEN
      ...
      -- Unit 8b: derive the KO stage type from bracket_type / consolation_bracket.
      -- consolation (via consolation_bracket->>'enabled') wins over the plain
      -- single/double choice, mirroring the flat-pool start_ko_phase detection
      -- (consolation is single_elimination main + a consolation sub-bracket).
      v_cons_enabled := coalesce((v_cons_cfg ->> 'enabled')::boolean, false)
                        AND coalesce(v_bracket_type, 'single_elimination')
                            <> 'double_elimination';
      ...
      IF v_cons_enabled THEN
        v_ko_type := 'consolation';
        -- main_bracket_size: honour the persisted value (ADR-0028 §5) when set;
        -- fall back to next_pow2(qualifier_count) otherwise.
        v_cons_main_size := coalesce((v_cons_cfg ->> 'main_bracket_size')::int, 0);
        IF v_cons_main_size < 2 THEN
          v_cons_main_size := 1;
          WHILE v_cons_main_size < v_qualifiers LOOP
            v_cons_main_size := v_cons_main_size * 2;
          END LOOP;
        END IF;
        v_cons_direct := greatest(0,
          coalesce((v_cons_cfg ->> 'direct_count')::int, 0));
        v_ko_cfg := jsonb_build_object(
          'ko_matchup',       coalesce(v_ko_matchup, 'seed_high_vs_low'),
          'with_third_place', v_third_place,
          'consolation', jsonb_build_object(
            'enabled',           true,
            'source',            'early_ko_losers',
            'main_bracket_size', v_cons_main_size,
            'direct_count',      v_cons_direct));
      ELSIF coalesce(v_bracket_type, 'single_elimination') = 'double_elimination' THEN
        v_ko_type := 'double_elim';
        -- with_reset: opt-in via ko_config.with_bracket_reset; owner default OFF,
        -- written EXPLICITLY into the stage config so the generator reads false
        -- rather than the legacy start_ko_phase default of true.
        v_with_reset := coalesce(
          (v_ko_config ->> 'with_bracket_reset')::boolean, false);
        v_ko_cfg := jsonb_build_object(
          'ko_matchup',  coalesce(v_ko_matchup, 'seed_high_vs_low'),
          'with_reset',  v_with_reset);
      ELSE
...
COMMENT ON FUNCTION public.tournament_start(uuid) IS
  'Start-RPC (re-based 20261313000000). ADR-0039 §4 (Unit 8b): the '
  'schoch_then_ko / swiss_then_ko auto-route now derives the KO stage TYPE from '
  'bracket_type / consolation_bracket — single_elim (default, byte-stable), '
  'double_elim (with_reset from ko_config.with_bracket_reset, owner default OFF '
  'written explicit), or consolation (config mirrors source early_ko_losers, '
  'main_bracket_size next_pow2(qualifier_count), direct_count). Seeding stays '
  'from_prev_ranking. Every other branch (round_robin_then_ko pool, swiss/schoch '
  'round 1, round_robin circle) is byte-identical to 20261313000000.';

-- ===================================================================
-- D) tournament_generate_stage_matches — re-based from 20261308000000. The
-- single_elim path stays BYTE-IDENTICAL; the consolation stage now ALSO
-- materialises the consolation sub-bracket via _tournament_compute_cons_bracket
-- (mirroring tournament_start_ko_phase). The double_elim branch is unchanged
-- (it now succeeds because the slot index carries phase). Every other branch
-- and the schedule/notify tail are byte-identical.
-- ===================================================================
...
DECLARE
  ...
  -- Unit 8b: consolation sub-bracket materialisation.
  v_cons_cfg       jsonb;
  v_cons_main_size int;
  v_cons_direct    int;
BEGIN
  ...
  ELSIF v_type IN ('single_elim', 'consolation') THEN
    ...
    -- Unit 8b: a consolation stage ALSO materialises its consolation sub-bracket
    -- skeleton (mirroring tournament_start_ko_phase). main_size =
    -- next_pow2(qualifier_count) == the main bracket size; the early_ko_losers
    -- are routed in by the advance trigger at runtime, so the R1 direct starters
    -- are empty here (source early_ko_losers, no direct seeding on the stage
    -- path). The cons rows carry stage_node_id so the stage-scoped advance
    -- (E) places winners in THIS stage's consolation bracket only. The
    -- consolation/consolation_third_place rows carry bracket_position so they
    -- get a slot in the phase-inclusive index (distinct phase from the main KO).
    IF v_type = 'consolation' THEN
      v_cons_cfg := v_config -> 'consolation';
      v_cons_main_size := coalesce((v_cons_cfg ->> 'main_bracket_size')::int, 0);
      IF v_cons_main_size < 2 THEN
        v_cons_main_size := 1;
        WHILE v_cons_main_size < v_n LOOP
          v_cons_main_size := v_cons_main_size * 2;
        END LOOP;
      END IF;
      v_cons_direct := greatest(0,
      ...
      SELECT p_tournament_id,
        ...
        FROM public._tournament_compute_cons_bracket(
               v_cons_main_size, '[]'::jsonb, '[]'::jsonb) c;
...
COMMENT ON FUNCTION public.tournament_generate_stage_matches(uuid, text, uuid[]) IS
  'ADR-0030 runner Step 3 (re-based 20261308000000). ADR-0039 §4 (Unit 8b): a '
  'consolation stage now materialises BOTH the main single-elim bracket and the '
  'consolation sub-bracket skeleton via _tournament_compute_cons_bracket '
  '(main_size next_pow2(qualifier_count), source early_ko_losers — the R1 losers '
  'are routed in by the advance trigger at runtime). The single_elim path is '
  'BYTE-IDENTICAL; double_elim now succeeds because the slot index carries phase '
  '(20261315000000). Every other branch and the schedule/notify tail are '
  'byte-identical to 20261308000000.';

-- ===================================================================
-- E) tournament_advance_ko_winner — re-based from 20261308000000. The DE and
-- consolation advance/drop branches gain
-- `AND stage_node_id IS NOT DISTINCT FROM NEW.stage_node_id` so a winner lands
-- only in its own stage bracket (no cross-stage corruption with two KO stages).
-- The NULL case (classic single-KO / start_ko_phase / round_robin_then_ko) is
-- behaviour-identical (NULL IS NOT DISTINCT FROM NULL = true). The type_graph
-- pre-dispatch guard and the single_elim / third_place paths are untouched.
-- ===================================================================
...
BEGIN
  ...
  -- =====================================================================
  -- SINGLE-ELIMINATION PATH (verbatim — byte-identical to 20261308000000).
  -- =====================================================================
  ...
  -- =====================================================================
  -- DOUBLE-ELIMINATION PATH. Unit 8b: stage-scoped so a winner lands only in
  -- its own stage's wb/lb/grand_final bracket.
  -- =====================================================================
  IF NEW.phase IN ('wb','lb','grand_final','grand_final_reset') THEN
    SELECT MAX(round_number) INTO v_wb_count
      ...
      WHERE tournament_id = NEW.tournament_id
        AND phase = 'wb'
        AND stage_node_id IS NOT DISTINCT FROM NEW.stage_node_id;
    v_size := (1 << v_wb_count);
  ...
  -- =====================================================================
  -- CONSOLATION ROUTING (E2, ADR-0028 §7.4). Unit 8b: stage-scoped.
  -- =====================================================================
...
COMMENT ON FUNCTION public.tournament_advance_ko_winner() IS
  'AFTER-UPDATE advance trigger (re-based 20261308000000). ADR-0039 §4 (Unit '
  '8b): the double_elim (wb/lb/grand_final) and consolation advance/drop '
  'branches are scoped with stage_node_id IS NOT DISTINCT FROM NEW.stage_node_id '
  'so a winner lands only in its own stage bracket (no cross-stage corruption '
  'with two KO stages). The NULL case (classic single-KO / start_ko_phase / '
  'round_robin_then_ko) is behaviour-identical (NULL IS NOT DISTINCT FROM NULL = '
  'true). type_graph routing and the single_elim / third_place paths are '
  'unchanged.';
