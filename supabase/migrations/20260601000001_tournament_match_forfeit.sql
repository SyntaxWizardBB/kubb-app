-- Tournament feature — Sprint A / W3-T1 forfeit-declaration RPC.
--
-- Covers DSCORE-62..-66 + FR-MATCH-7 + FR-CFG-11 + BR-14:
-- the organizer can declare a no-show forfeit from the match-detail
-- surface, the absent side is recorded, a free-text reason
-- (>= 10 chars) is captured, the score is filled in automatically
-- from `tournaments.forfeit_points`, and the match flips to
-- `finalized` ("finished" in spec wording — the schema uses the
-- `finalized` enum value, see 20260525000001_tournament_schema.sql).
--
-- Status gating: the tournament must be in `live` status (the schema
-- enum equivalent of the spec's "running" state). The match itself
-- must be in a not-yet-finalised status (`scheduled`, `awaiting_results`
-- or `disputed`); already-`finalized`, `overridden` or `voided` rows
-- are immutable per the consensus engine.
--
-- Audit-log: this migration writes the canonical `match_forfeit_declared`
-- event into `tournament_audit_events` (the existing table — there is
-- no dedicated `match_audit_log` table yet). The Sprint A audit-log
-- sweep (separate task) is the one that owns the cross-feature audit
-- consolidation; the marker below pins the entry-point for that sweep.
...
BEGIN
  ...
  -- Status gate: only the tournament creator (organizer) may declare a
  -- forfeit, and only while the tournament is live (spec: "running").
  SELECT t.created_by, t.status, t.forfeit_points
    INTO v_creator, v_t_status, v_forfeit_points
