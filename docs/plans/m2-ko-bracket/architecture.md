...
- `tournament_start_ko_phase(p_tournament_id uuid)` — pur server-seitig: liest die aktuellen Standings (computed clientseitig in M1, aber beim KO-Start brauchen wir Server-Authority, weil sonst zwei Veranstalter-Geräte konkurrieren könnten), validiert das Bracket, inserted die Match-Rows mit `phase='ko'`. Erwartet, dass alle Vorrunden-Matches `finalized` oder `overridden` sind. Setzt `tournaments.status` auf `live` (falls noch nicht) und schreibt ein Audit-Event `phase_started`. **Authority-Modell: Server-Authority per Committee-OD-M2-02 (resolved 2026-05-26).** Folgt strikt dem `tournament_start`-Pattern: `FOR UPDATE` auf `tournaments`-Row, Idempotency-Guard via `EXISTS (... phase <> 'group')` mit `ERRCODE 40001` — der Dart-Client behandelt diesen Code als idempotente Success-Semantik (`ref.invalidate` ohne Error-Toast). Bracket-Generierung läuft via separaten Helper `_tournament_compute_ko_bracket(seeds jsonb, third_place bool)`, der den Recursive-Standard-Seeding-Algorithmus aus `bracket.dart` 1:1 nach plpgsql portiert (Property-Parität-Tests als Merge-Gate). Siehe ADR-0017 §7.
...
Hinweis zur Server-Authority: Die Bracket-Generierung läuft im Server als plpgsql-Implementation (Committee-OD-M2-02, resolved 2026-05-26). Die pure Dart-Implementation in `bracket.dart` bleibt als Domain-Modell und Test-Referenz, die plpgsql-Version ist eine 1:1-Spiegelung mit Property-Parität-Tests als Merge-Gate gegen Drift. Race-Condition zwischen zwei parallelen Veranstalter-Geräten ist via `FOR UPDATE` plus `ERRCODE 40001`-Idempotency-Pattern adressiert.
...
- ~~**Server-Authority für Bracket-Generation**~~: resolved 2026-05-26 (Committee OD-M2-02) — Server-Authority via plpgsql-Spiegelung ist verbindlich, siehe §3.2 und ADR-0017 §7.
